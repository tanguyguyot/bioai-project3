...
import numpy as np
from cuml.ensemble import RandomForestClassifier  # GPU-accelerated RF
from cuml.model_selection import train_test_split
from cuml import metrics
from joblib import Parallel, delayed

def get_table(dataset, feature_columns, y, penalty_factor=0.01, test_size=0.3, max_features_subset=5) -> dict:
    error_table = {}

    def train_subset(selected_columns):
    ...
    for i in tqdm(range(1, min(len(feature_columns), max_features_subset) + 1)):
        results = Parallel(n_jobs=-1)(delayed(train_subset)(cols) for cols in combinations(feature_columns, i))
        error_table.update(results)
